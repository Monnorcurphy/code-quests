...
export default function QuestRoute() {
  ...
  if (isLoading) {
    return (
      <main style={{ width: '100vw', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <p aria-live="polite" className="text-gray-700">Loading quest…</p>
      </main>
    );
  }
  ...
  if (isError || !quest) {
    return (
      <main
        style={{ width: '100vw', height: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px' }}
        aria-live="polite"
      >
        <p className="text-gray-700">
          {is404 ? 'Quest not found — return to town' : 'Could not load quest. Make sure the server is running.'}
        </p>
        <button
          onClick={() => navigate('/town/town-square')}
          className="text-gray-700"
          style={{ padding: '8px 16px', border: '1px solid currentColor', borderRadius: '4px', cursor: 'pointer' }}
        >
          Return to Town
        </button>
      </main>
